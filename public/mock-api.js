...
class MockApi {
  ...
  init() {
    ...
    if (this.isActive) {
      console.log(
        "Mock API is active - all API calls will be handled client-side"
      );
    ...
    try {
      ...
      if (action === "approve") {
        return this.createSuccessResponse({
          success: true,
          message:
            "Push notification approved. You have been authenticated successfully!",
        });
      } else if (action === "deny") {
        return this.createSuccessResponse({
          success: false,
          message:
            "Authentication request denied. For security reasons, please try again.",
        });
      } else {
  ...
  handleMagicLinkGeneration(options) {
    try {
      ...
      const token = Array.from(Array(32), () =>
      ...
      const magicLinkUrl = `https://secureauth.example.com/verify?token=${token}&email=${encodeURIComponent(
        email
      )}`;
